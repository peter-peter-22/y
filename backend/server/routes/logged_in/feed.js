...
router.post("/get_posts", async (req, res, next) => {
    await post_list(req, res, undefined, undefined, " WHERE post.replying_to IS NULL");
});
router.post("/get_followed_posts", async (req, res, next) => {
    await post_list(req, res, undefined, undefined, " WHERE post.replying_to IS NULL AND post.publisher=ANY(SELECT follows.followed from follows WHERE follows.follower=:user_id)");
});
